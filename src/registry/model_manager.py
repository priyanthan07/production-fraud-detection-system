...
def promote_to_production(client: MlflowClient, version: str) -> None:
    ...
    logger.info(
        f"{MODEL_NAME} version {version} is now in Production. Previous Production version has been archived."
    )
...
def run_promotion_workflow(
    ...
    for gate_name, result in gate_report.items():
        ...
        logger.info(
            f"  [{status}] {gate_name}: {result['actual']} >= {result['required']} required"
        )
