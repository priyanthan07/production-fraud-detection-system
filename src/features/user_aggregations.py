...
def compute_user_aggregations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["card1_amt_zscore"] = np.where(
        df["card1_amt_std"]>0,
        df["card1_amt_deviation"] / df["card1_amt_std"],
        0.0
    )
    ...
    df["email_amt_deviation"] = (
        df["TransactionAmt"] - df["email_amt_mean"]
    )
    
    df["email_amt_zscore"] = np.where(
        df["email_amt_std"]>0,
        df["email_amt_deviation"] / df["email_amt_std"],
        0.0   
    )
