...
class FraudPredictor:
    ...
    def load(self, model_uri: str = None) -> None:
        ...
        if CONFIG_PATH.exists():
            ...
            tracking_uri = self.config.get("mlflow_tracking_uri", "postgresql://postgres:admin@localhost:5432/mlflow_tracking")
    ...
    def _load_version_and_threshold(self, model_uri: str) -> str:
        ...
        try:
            ...
            if stage_or_version.isdigit():
                ...
                logger.warning(f"No version found for stage {stage_or_version}. Using default threshold 0.5.")
            ...
            if threshold_str is not None:
                ...
                logger.info(f"Loaded threshold {self.threshold:.4f} from MLflow run {run_id[:8]}...")
            else:
                logger.warning("optimal_threshold not found in run params. Using default 0.5.")
                self.threshold = 0.5
