...
class FraudPredictor:
    ...
    def _load_version_and_threshold(self, model_uri: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Could not fetch version/threshold from MLflow: {e}. Using defaults.")
            self.model_version = "unknown"
            self.threshold = 0.5
    ...
    def _select_model_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing:
            logger.debug(
                f"{len(missing)} feature columns missing, filling with NaN. First 5: {missing[:5]}"
            )
    ...
    def predict_batch(
        ...
        logger.info(f"Batch complete. {flagged}/{len(transactions)} flagged.")
        return results
