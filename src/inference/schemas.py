...
class TransactionInput(BaseModel):
    ...
    ProductCD: Optional[str] = Field(None, description="Product code", json_schema_extra={"example": "W"})
    card1: Optional[int] = Field(None, description="Card identifier 1", json_schema_extra={"example": 13926})
    ...
    card4: Optional[str] = Field(None, description="Card network", json_schema_extra={"example": "visa"})
    card5: Optional[float] = Field(None, json_schema_extra={"example": 226.0})
    card6: Optional[str] = Field(None, description="Card type", json_schema_extra={"example": "debit"})
 
    # --- Address fields ---
    addr1: Optional[float] = Field(None, description="Billing address zip", json_schema_extra={"example": 315.0})
    addr2: Optional[float] = Field(None, description="Billing address country", json_schema_extra={"example": 87.0})
    ...
    # --- Email domains ---
    P_emaildomain: Optional[str] = Field(None, description="Purchaser email domain", json_schema_extra={"example": "gmail.com"})
    R_emaildomain: Optional[str] = Field(None, description="Recipient email domain", json_schema_extra={"example": "gmail.com"})
...
class BatchTransactionInput(BaseModel):
    ...
    def check_transaction_ids_unique(self) -> "BatchTransactionInput":
        ...
        if len(ids) != len(set(ids)):
            raise ValueError("All TransactionIDs in a batch must be unique. Duplicate IDs detected.")
        return self
